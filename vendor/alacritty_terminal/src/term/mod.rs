...
impl<T: EventListener> Handler for Term<T> {
    ...
    fn input(&mut self, c: char) {
        ...
        if width == 0 {
            ...
            // SolidTerm patch: Variation Selector-16 (U+FE0F) requests emoji
            // presentation, which is full-width. Promote the just-printed
            // base cell to a wide char and lay a spacer in the trailing cell
            // (the cursor is parked there), so the renderer reserves two
            // columns — matching kitty / Ghostty. Only the common non-wrap
            // case: if the base is already wide, the line wrapped, or the
            // cursor isn't parked immediately after the base, fall through
            // to plain zero-width handling.
            if c == '\u{FE0F}'
                && !self.grid.cursor.input_needs_wrap
                && self.grid.cursor.point.column.0 == column.0 + 1
                    ...
                    .intersects(Flags::WIDE_CHAR | Flags::WIDE_CHAR_SPACER)
            {
                ...
                if self.grid.cursor.point.column.0 + 1 < self.columns() {
