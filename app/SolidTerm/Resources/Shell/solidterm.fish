...
if set -q _NEXTTERM_INTEGRATION_LOADED
    exit
end
set -g _NEXTTERM_INTEGRATION_LOADED 1
...
function _solidterm_prompt_start --on-event fish_prompt
    ...
    if set -q _NEXTTERM_COMMAND_STARTED
        printf '\e]133;D;%d\a' $status
        set -e _NEXTTERM_COMMAND_STARTED
    end
    printf '\e]133;A\a'
end
...
function _solidterm_preexec --on-event fish_preexec
    set -g _NEXTTERM_COMMAND_STARTED 1
    printf '\e]133;C\a'
end
