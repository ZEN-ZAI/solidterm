...
final class FrameDeltaDecodingPerfTests: XCTestCase {
    ...
    // Ceiling sized to catch the catastrophic regression (re-introducing the
    // per-byte `vec.get(index:)` FFI loop, which would push 100 iterations into
    // multi-second territory) while staying loose enough for `xcodebuild test`'s
    // Debug build, where per-cell array allocations in the decoder dominate over
    // the FFI boundary cost. Real Release-build budgets land alongside #8.
    private static let ceilingSeconds: Double = 1.5
    ...
    func testDecodeCellsRustVecHotPathBudget() throws {
        ...
        print(
            "decodeCells(RustVec<UInt8>) ×\(Self.iterations) over \(Self.cellsPerFrame) cells: "
                + String(format: "%.4f", elapsed * 1000) + " ms")
        XCTAssertLessThan(
            elapsed, Self.ceilingSeconds,
            "decodeCells(RustVec<UInt8>) regressed past the "
                + String(format: "%.0f", Self.ceilingSeconds * 1000)
                + " ms ceiling — check for re-introduction of an O(n) per-byte FFI loop")
    }
}
